...
class PrunedResidualUNetDS(nn.Module):
    ...
    def __init__(
        self, in_ch: int = 1, base_ch: int = 32, channel_counts: dict = None, aux_d3_ch: int = None
    ):
        super().__init__()
...
class TemporalFetaSegNet(nn.Module):
    ...
    def forward(self, clip: torch.Tensor):
        ...
        logits = self.backbone.decode(bn_att.view(B * T, Cb, hb, wb), skips).view(B, T, 1, H, W)
...
def load_phase4a(ckpt_path: str, device: torch.device = DEVICE) -> PrunedResidualUNetDS:
    ...
    model = PrunedResidualUNetDS(
        in_ch=1, base_ch=32, channel_counts=channel_counts, aux_d3_ch=aux_d3_ch
    )
    model.load_state_dict(ckpt["model_state_dict"])
    return model.to(device).eval()
...
def validate_input(img_gray: np.ndarray) -> dict:
    ...
    if img_gray.ndim != 2:
        checks["shape"] = False
        warnings.append(
            "Image must be 2D grayscale (received shape: "
    ...
    if not checks["resolution"]:
        warnings.append(
            f"Image resolution ({w}×{h}) is below the 64×64 minimum. Results will be unreliable."
        )
    ...
    if not checks["not_blank"]:
        warnings.append(
            "Image appears to be blank (near-black). Check that the correct file was uploaded."
        )
    if not checks["not_saturated"]:
        warnings.append(
            "Image appears to be saturated (near-white). This is unusual for ultrasound images."
        )
    ...
    if not checks["dynamic_range"]:
        warnings.append(
            f"Image has very low dynamic range (std={std_int:.1f}). "
    ...
    if not checks["has_texture"]:
        warnings.append(
            "Image has very low spatial texture. "
            "This model was trained on real ultrasound frames — "
            "synthetic or heavily processed images may produce poor results."
        )

    # 7. Image quality scoring (Batch 8.3) — Laplacian variance is a robust
    # blur proxy; combined with dynamic-range and aspect-ratio penalties
    # this gives a 0–1 quality score and a 4-tier label that the UI can
    # surface as a quality badge before / after inference.
    quality_score = _compute_quality_score(lap_var, std_int, mean_int, h, w)
    quality_label = _quality_label(quality_score)
    if quality_label in ("poor", "suboptimal"):
        warnings.append(
            f"Image quality {quality_label} (score {quality_score:.2f}). "
            "Consider re-acquiring at the standard biometric plane with optimal "
            "gain / focal-zone settings for higher-confidence measurement."
        )
    ...
    return {
        ...
        "blur_score": lap_var,  # raw Laplacian variance for downstream consumers
    }


def _compute_quality_score(
    lap_var: float, std_int: float, mean_int: float, h: int, w: int
) -> float:
    """Composite 0–1 quality score combining four heuristics.

    Each component is mapped to a 0–1 sub-score and the result is the
    weighted average:
      blur (60%)          — Laplacian variance, the standard blur proxy
      contrast (20%)      — pixel intensity std dev (broad dynamic range)
      brightness (10%)    — mean intensity not too dark / not too bright
      resolution (10%)    — penalised when much smaller than the 256x384 input
    """
    # Blur component — Laplacian variance saturates around 300 on clean
    # ultrasound frames; below 30 the image is essentially out of focus.
    blur = max(0.0, min(1.0, (lap_var - 20.0) / 280.0))

    # Contrast — std around 60+ is healthy ultrasound; saturate at 80
    contrast = max(0.0, min(1.0, std_int / 80.0))

    # Brightness — penalise far from mid-grey (~120)
    brightness = max(0.0, 1.0 - abs(mean_int - 120.0) / 120.0)

    # Resolution — full credit at >=256 on the short axis
    short_axis = min(h, w)
    resolution = max(0.0, min(1.0, short_axis / 256.0))

    return round(0.60 * blur + 0.20 * contrast + 0.10 * brightness + 0.10 * resolution, 3)
...
def predict_single_frame(
    model: ResidualUNetDS | PrunedResidualUNetDS,
    img_gray: np.ndarray,
    pixel_spacing_mm: float,
