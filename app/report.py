...
def _styles(llm: bool = False):
    ...
    return dict(
        title = ParagraphStyle("ti", parent=s["Heading1"], fontSize=13,
                               spaceAfter=3, textColor=_NAVY),
        sub   = ParagraphStyle("su", parent=s["Normal"], fontSize=8.5,
                               spaceAfter=8, textColor=_GREY),
        badge = ParagraphStyle("ba", parent=s["Normal"], fontSize=8,
                               spaceAfter=6, textColor=acc,
                               fontName="Helvetica-Bold"),
        sec   = ParagraphStyle("se", parent=s["Heading2"], fontSize=10.5,
                               spaceBefore=10, spaceAfter=3, textColor=_NAVY),
        body  = ParagraphStyle("bo", parent=s["Normal"], fontSize=9.5,
                               spaceAfter=5, leading=14.5),
        bodyI = ParagraphStyle("bi", parent=s["Normal"], fontSize=9.5,
                               spaceAfter=5, leading=14.5, textColor=_GREY),
        warn  = ParagraphStyle("wa", parent=s["Normal"], fontSize=8.5,
                               leading=12, textColor=_AMBER),
        label = ParagraphStyle("la", parent=s["Normal"], fontSize=9,
                               spaceAfter=2, fontName="Helvetica-Bold",
                               textColor=_NAVY),
        green = ParagraphStyle("gr", parent=s["Normal"], fontSize=9,
                               spaceAfter=5, leading=13, textColor=_PRUNED),
    )
...
def _hr(story, color=None, thickness=0.5):
    story.append(HRFlowable(
        ...
        color=color or _BORDER, spaceAfter=6,
    ))
...
def _report_header(story, st, title_line, model_name, report_type_label, llm, elapsed_ms=None):
    ...
    if m["pruned"]:
        badge_parts.append(
            f'<font color="{_PRUNED.hexval()}">'
            f'[COMPRESSED MODEL — {m["compression"]}]</font>'
        )
    badge_parts.append(
        f'<font color="{_NAVY.hexval()}">[{report_type_label}]</font>'
    )
...
def _llm_static_narrative(hc, ga_str, ga_weeks, trim, gradcam_ok,
                           model_name, elapsed_ms, api_key):
    m   = _meta(model_name)
    ctx = {
...
def _llm_cine_narrative(hc, ga_str, ga_weeks, trim, rel, std,
                         n_frames, model_name, elapsed_ms, api_key):
    m       = _meta(model_name)
...
def _rule_cine_p1(hc, ga_str, ga_weeks, trim, rel, std):
    rel_desc = "excellent" if rel > 0.97 else "good" if rel > 0.93 else "moderate"
    std_desc = "highly stable (<2 mm inter-frame deviation)" if std < 2.0 else \
               f"acceptably stable ({std:.1f} mm inter-frame deviation)" if std < 5.0 else \
               f"variable ({std:.1f} mm inter-frame deviation — review recommended)"
    ctx = {
        "Early (<20w)":  "consistent with first-trimester biometric parameters",
        "Mid (20–30w)":  "within the optimal second-trimester sonographic assessment window",
...
def _biometric_table(story, st, hc, ga_str, trim, conf_label, model_name, elapsed_ms, pixel_spacing):
    m = _meta(model_name)
    ...
    rows = [
        ...
        ["Estimated Gestational Age",
         ga_str or "—",
         "Hadlock (1984) nomogram — ±2 weeks CI"],
        ["Trimester Classification",
         trim or "—",
         "Derived from estimated GA"],
    ...
    t.setStyle(_tbl_style()); story.append(t); story.append(Spacer(1, 4 * mm))
...
def _model_performance_table(story, st, model_name, elapsed_ms):
    ...
    rows = [
        ...
        ["Segmentation accuracy (validation)",
         m["dice"],
         f"HC18 cohort — {m['dataset']}"],
        ["Mean measurement deviation",
         m["mae"],
         "ISUOG acceptable biometry threshold: ±3 mm"],
        ["ISUOG clinical threshold",
         f"✓ {m['isuog']}",
         "ISUOG Practice Guidelines 2010"],
        ["Model computational footprint",
         m["params"],
         "Parameter count"],
...
def _temporal_table(story, st, rel, std, n_frames):
    rel_label = "Excellent (>0.97)" if rel > 0.97 else \
                "Good (0.93–0.97)" if rel > 0.93 else "Moderate (<0.93)"
    std_label = "Highly stable" if std < 2.0 else \
                "Acceptable" if std < 5.0 else "Variable — review advised"
    ...
    rows = [
        ...
        ["Inter-frame concordance", rel_label,
         "Consistency of calvarium boundary across frames"],
        ["Frame-to-frame HC variability", f"{std:.2f} mm", std_label],
        ["Consensus method",
         "Temporal mean probability",
         "Mean prediction across all frames"],
    ]
    t = Table(rows, colWidths=[58 * mm, 42 * mm, 70 * mm])
    t.setStyle(_tbl_style()); story.append(t); story.append(Spacer(1, 4 * mm))
...
def generate_static_report(
    ...
    _report_header(story, st,
                   "Fetal Head Circumference — Automated Biometry Report",
                   model_name, "STATIC SINGLE-FRAME ANALYSIS", llm, elapsed)
    ...
    if llm:
        ...
        story.append(Paragraph("<b>Biometric assessment</b>", st["label"]))
        story.append(Paragraph(_rule_static_p1(hc, ga_str, ga_weeks, trim), st["body"]))
        story.append(Spacer(1, 2 * mm))
        story.append(Paragraph("<b>Activation map interpretation</b>", st["label"]))
        story.append(Paragraph(_rule_static_p2(gradcam), st["body"]))
        comp_note = _rule_compression_note(model_name, elapsed)
        if comp_note:
            story.append(Spacer(1, 2 * mm))
            story.append(Paragraph("<b>Computational efficiency note</b>", st["label"]))
            story.append(Paragraph(comp_note, st["green"]))
...
def generate_cine_report(
    ...
    _report_header(story, st,
                   "Fetal Head Circumference — Automated Biometry Report",
                   model_name, "TEMPORAL CINE-LOOP ANALYSIS", llm, elapsed)
    ...
    if llm:
        ...
        story.append(Paragraph("<b>Biometric assessment and temporal concordance</b>", st["label"]))
        story.append(Paragraph(_rule_cine_p1(hc, ga_str, ga_weeks, trim, rel, std), st["body"]))
        story.append(Spacer(1, 2 * mm))
        story.append(Paragraph("<b>Cine-loop analysis — clinical rationale</b>", st["label"]))
        story.append(Paragraph(_rule_cine_p2(rel, std, n_frames), st["body"]))
        comp_note = _rule_compression_note(model_name, elapsed)
        if comp_note:
            story.append(Spacer(1, 2 * mm))
            story.append(Paragraph("<b>Computational efficiency note</b>", st["label"]))
...
def generate_comparison_report(
    ...
    story.append(Paragraph(
        "Fetal Head Circumference — Four-Model Comparative Biometry Report", st["title"]))
    story.append(Paragraph(
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC  |  "
        f"All four AI model variants evaluated on identical input image  |  "
        f"Pixel spacing: {pixel_spacing:.4f} mm/pixel", st["sub"]))
    story.append(Paragraph(
    ...
    cmp_rows = [
        ...
        ["Analysis type", "Single frame", "Single frame", "16-frame consensus", "16-frame consensus"],
    ]
    ...
    if llm:
        ...
        story.append(Paragraph(
            "<b>Static vs temporal analysis — clinical recommendation</b>", st["label"]))
        story.append(Paragraph(p1, st["body"]))
        story.append(Spacer(1, 3 * mm))
        story.append(Paragraph(
            "<b>Compressed model variants — deployment significance</b>", st["label"]))
...
def generate_pdf_report(
    ...
    if result.get("mode") == "cine_clip":
        return generate_cine_report(result, api_key=api_key, use_llm=use_llm,
                                    model_name=model_name or "Phase 2 — Temporal baseline",
                                    pixel_spacing=pixel_spacing)
    return generate_static_report(result, api_key=api_key, use_llm=use_llm,
